...
import com.justep.business.server.BusinessServer;
...
public class OPMProcedure {
	...
	public static Table query(String concept, String idColumn, String select, String from, String condition, List<DataPermission> range, String filter, Boolean distinct, int offset, int limit,
		...
		if (includePersonPhotoLastModified) {
			...
			select = select + ",SA_OPPerson.sPhotoLastModified as sPhotoLastModified,SA_OPPerson.sNumb as sNumb,SA_OPPerson.sGlobalSequence as sGlobalSequence";
	...
	private static Object login(String name, Date loginDate, String pwd, String lang, boolean checkPwd, String ip, Map<Object, Object> options) {
		...
		if ("true".equals(result.get(0))) {
			...
			// 单一用户登录
			Config singleUserLogin = (Config) ModelUtils.getModel("/system/config").getLocalObject("singleUserLogin", Config.TYPE);
			if (singleUserLogin != null && "true".equals(singleUserLogin.getValue())) {
				Iterator<User> itor = UserManager.instance().getUsers().iterator();
				String sessionID = ContextHelper.getSessionContext().getSessionID();
				while (itor.hasNext()) {
					User user = itor.next();
					if (user.getID().equals(operator.getID()) && !user.getSessionID().equals(sessionID)) {
						try {
							UserManager.instance().removeUser(sessionID);
							BusinessServer.instance().unregistSession(ContextHelper.getSessionContext().getSession(), sessionID);
							LicenseChecker.instance().deleteOnlineUser(sessionID);
						} catch (Exception e) {
						}
						ContextHelper.getSessionContext().getSession().invalidate();
						throw new BusinessException("您的帐号于" + new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(user.getLoginDate()) + "在" + user.getLoginIP() + "登录，请先注销登录");
					}
				}
			}
